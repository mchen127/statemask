...
def make_env():
    ...
    def _thunk():
        env = gym.make("ALE/Pong-v5", render_mode="rgb_array").env
        return env
...
def main():
    ...
    for i in tqdm(range(N_TESTS), desc="Testing baseline model"):
        total_reward, total_disc_reward, count, _, _ = test_episode(
            env, baseline_model, device
        )
    ...
    for i in tqdm(range(N_TESTS), desc="Testing mask network"):
        total_reward, _, _, _, _ = test_episode(
            env, mask_network, device, record_video=True, video_prefix=f"vid_mask_{i}"
        )
        tmp_rewards.append(total_reward)
    print("Average winning rate: ", np.mean(tmp_rewards))
    ...
    for i in tqdm(range(N_TESTS), desc="Generating fid tests"):
        total_reward, total_disc_reward, count, action_seq, mask_probs = test_episode(
            env, baseline_model, device
        )
        tmp_rewards.append(total_reward)
