...
def test_statemask(baseline_model, statemask_model, env_id, num_episodes=500):
    # Record start time
    ...
    for episode in tqdm(range(num_episodes), desc="Testing episodes"):
        ...
        while not done:
            ...
            baseline_dist, baseline_value = baseline_model(state_reorder)
            baseline_action = baseline_dist.sample()
            
            statemask_dist, statemask_value = statemask_model(state_reorder)
            statemask_action = statemask_dist.sample()
            
            real_action = baseline_action if not statemask_action else np.random.choice(6)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--statemask_model_path", type=str, required=True, help="Path to the trained model"
    )
    parser.add_argument(
    ...
    statemask_model = PPO(
        num_inputs=1, num_outputs=2, hidden_size=256
    )  # Adjust these based on your setup
    statemask_model.load_state_dict(torch.load(args.statemask_model_path)["state_dict"])
    statemask_model.eval()

    # Run the test
    results = test_statemask(
